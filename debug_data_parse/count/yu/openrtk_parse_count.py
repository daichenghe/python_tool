...
class DeubgAllParse:
	def __init__(self, data_file, path):
		self.debug_all_data = []
		if is_later_py_3:
			self.debug_all_data = data_file.read()
		else:
			self.filedata = data_file.read()
			for c in self.filedata:
				self.debug_all_data.append(ord(c))
		self.path = path
		self.packet_buffer = []  
		self.packet_buffer_useful = []
		self.sync_state = 0
		self.sync_pattern = collections.deque(6*[0], 6)  
		self.debugAllPacketsTypeList = []
		self.log_files = {}
		self.fp_all = None
	...
	def start_pasre(self):
		self.debugAllPacketsTypeList = self.rtk_properties['debugAllPacketsTypeList']
		count = 0
		length = 0
		for i,new_byte in enumerate(self.debug_all_data):
			self.sync_pattern.append(new_byte)
			if self.sync_state == 1:
				self.packet_buffer.append(new_byte)
				packet_buff_len = len(self.packet_buffer)
				if (self.cur_message_id == '$GPIMU'):
					if (self.packet_buffer[packet_buff_len-2]) == ord('\r') and self.packet_buffer[packet_buff_len-1] == ord('\n'):
						try:
							for ele in self.packet_buffer:
								self.packet_to_str += chr(ele)
							self.time_tag = self.packet_to_str.split(',')[1]
							#print(bytes(self.packet_buffer))
						...
						self.packet_buffer = []
						self.sync_state = 0
				elif(self.cur_message_id == '$GPGGA'):
					if (self.packet_buffer[packet_buff_len-2]) == ord('\r') and self.packet_buffer[packet_buff_len-1] == ord('\n'):
						#print(bytes(self.packet_buffer))
						self.parse_output_packet_payload(packet_type)
						self.packet_buffer = []
						self.sync_state = 0
				elif(self.cur_message_id == '$GPROV'):
					split_num = str(bytes(self.packet_buffer)).count(',')
					if(split_num == 3):
						length = (str(bytes(self.packet_buffer)).split(','))[2]
						time =	(str(bytes(self.packet_buffer)).split(','))[1]
					if(split_num >= 3):
						count+= 1
						if(int(length) == count):
							packet_buff_len = len(self.packet_buffer)
							if (self.packet_buffer[packet_buff_len-2]) == ord('\r') and self.packet_buffer[packet_buff_len-1] == ord('\n'):
								#print(bytes(self.packet_buffer))
								self.packet_buffer_useful = self.packet_buffer[packet_buff_len-count+1:packet_buff_len-5]
								#print(bytes(self.packet_buffer))
								#print(bytes(self.packet_buffer_useful))
								self.parse_output_packet_payload(packet_type)
								self.packet_buffer = []
								self.sync_state = 0		
							else:
							...
							count = 0
							split_num = 0
				elif(self.cur_message_id == '$GPREF'):
					split_num = str(bytes(self.packet_buffer)).count(',')
					if(split_num == 3):
						length = (str(bytes(self.packet_buffer)).split(','))[2]
					if(split_num >= 3):
				...
				packet_type = ''
				for message_id in self.debugAllPacketsTypeList:
					message_id_list = []				
	...
	def start_log(self, output):
		...
		if output['name'] not in self.log_files.keys():
			if output['name'] == 'IMU':
				...
				self.log_files[output['name'] + '_bin'] = open(self.path + output['name'] + '.bin', 'wb')
				self.write_titlebar(self.log_files[output['name']], output)
			else:
				self.log_files[output['name']] = open(self.path + output['name'] + '.bin', 'wb')
	...
	def log(self, output, data):
		...
		if name == 'IMU':
			for i in range(1,len(data_list)-1):
				self.write_data_fm(name, data_list[i],'')
				self.write_data(name, ",")
			self.write_data(name, "\n")
			self.log_one_packet(name+'_bin',self.packet_buffer)
		elif name == 'NEMA':
			self.log_one_packet(name,self.packet_buffer)
		elif name == 'ROVER' or name == 'BASE':
			self.log_one_packet(name,self.packet_buffer_useful)
		
		#self.write_data(name, "\n")


	def log_one_packet(self, name,data):
		self.log_files[name].write(bytes(data))
	def parse_output_packet_payload(self, message_id):
		'''debug all packet'''
		if message_id == '$GPIMU':
			payload = self.packet_to_str
		output = next((x for x in self.rtk_properties['debugAllPackets'] if x['messageId'] == str(message_id)), None)
		if output != None:
			self.start_log(output)
			if self.packet_to_str != None:
				self.log(output, self.packet_to_str)
		else:
	...
	def calc_block_lrc(self, payload):
		ulCRC = 0
...
if __name__ == '__main__':
	...
	for root, dirs, file_name in os.walk(args.p):
		for fname in file_name:
			if (fname.startswith('user') or fname.startswith('debug_all')) or fname.endswith('.bin'):
				file_path = os.path.join(root, fname)
				print('processing {0}'.format(file_path))
				path = mkdir(file_path)
				try:
					with open(file_path, 'rb') as fp_rawdata:
						if fname.startswith('debug_all'):
							print('start parse')
							parse = DeubgAllParse(fp_rawdata, path + '/' + fname.rstrip(".bin") + '_')
						'''
