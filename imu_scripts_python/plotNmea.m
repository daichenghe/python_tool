...
f = dir([dirpath, 'gga_*.log']);

for i = 1:length(f)

    filename = [dirpath, f(i,1).name];
    fa = strsplit(filename, '.log');
    of = strcat(fa{1}, '.pos');
    
    [hour, minute, second, velocity, numsats, height, lat, long, quality] = readNmea(filename);
    
    pos = [lat, long, height, numsats, quality];
    fid = fopen((of), 'wt');
    if fid
        
       fprintf(fid, '%.10f,%.10f,%.3f,%d,%d\n', pos'); 
    end
    fclose(fid);
